...
const GoalForm: FC<GoalFormProps> = ({
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>): void => {
    ...
    if (!profileBirthday) {
      setError('Please add your birthday in your profile before creating a goal')
      onOpenProfile()
      return
    }
    ...
    return (
      ...
      <form className="goal-form" onSubmit={handleSubmit}>
        <div className="form-section">
          ...
          <div className="form-group">
            <label htmlFor="goalCreatedIn">Goal Created on</label>
            <input
              type="date"
              id="goalCreatedIn"
              name="goalCreatedIn"
          ...
          <div className="form-group">
            <label htmlFor="goalEndYear">Goal End Year</label>
            <input
            type="date"
              id="goalEndYear"
              name="goalEndYear"
          ...
          {formData.goalCreatedIn && (
            <div className="display-field">
              ...
              <span className="display-value">{formatMonthYear(formData.goalCreatedIn)}</span>
            </div>
          )}
        ...
        <div className="form-section">
          ...
          <div className="form-group">
            <label htmlFor="expenseValue">Annual Expense ({formatMonthYear(formData.goalCreatedIn)}) </label>
            <input
              type="text"
              inputMode="numeric"
              id="expenseValue"
          ...
          {formData.expenseValue && formData.goalCreatedIn && (
            <>
              <div className="display-field">
                <span className="label">Monthly Expense at creation ({formatMonthYear(formData.goalCreatedIn)}):</span>
                <span className="display-value">
                  ${(Number(formData.expenseValue) / 12).toLocaleString(undefined, { maximumFractionDigits: 2 })}
                </span>
              </div>
              <div className="display-field">
                <span className="label">Annual Expense at creation ({new Date(formData.goalCreatedIn).getFullYear()}):</span>
                <span className="display-value">
          ...
          {formData.expenseValue && profileBirthday && formData.retirementAge && formData.goalCreatedIn && (
            (() => {
              const metrics = calculateGoalMetrics(
