...
const GoalFilterBar: FC<GoalFilterBarProps> = ({ goals, filters, onChange }) => {
  ...
  const availableExpenseBuckets = EXPENSE_BUCKETS.filter(b => goals.some(p => p.expenseValue >= b.min && p.expenseValue < b.max))
    ...
    <div className="goal-filter-bar">
      ...
      {hasFilters && (
        <div className="goal-filter-right">
          <span className="goal-filter-active-count">{activeCount} filter{activeCount > 1 ? 's' : ''} active</span>
          <button className="goal-filter-clear-all" onClick={clearAll}>Clear all</button>
        </div>
      )}
    </div>
  )
