...
const AdvancedPane: FC<AdvancedPaneProps> = ({ allowCsvImport, onToggleAllowCsvImport, onExport, onImport, onFactoryReset, onClose }) => {
    ...
    <div className="settings-section">
      ...
      <div className="settings-section-content">
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
          ...
          <input ref={importFileInputRef} type="file" accept=".json" onChange={handleImportFileChange} style={{ display: 'none' }} />
        </div>

        {!resetConfirmOpen ? (
          <button className="settings-btn settings-btn--danger" onClick={() => setResetConfirmOpen(true)}>Factory Reset App</button>
        ) : (
          <div className="settings-reset-confirm">
            ...
            <div className="settings-reset-actions">
              <button className="settings-btn settings-btn--outline" onClick={() => setResetConfirmOpen(false)}>Cancel</button>
              <button className="settings-btn settings-btn--danger" onClick={() => { onFactoryReset(); onClose() }}>Yes, Reset Everything</button>
            </div>
          </div>
        )}
      </div>
