...
describe('getLatestGoalTotals', () => {
  ...
  it('returns zeros when accounts exist but no balances', () => {
    expect(getLatestGoalTotals([makeAccount()], [])).toEqual({ fiTotal: 0, gwTotal: 0 })
  })
  ...
  it('computes FI total from latest month balances', () => {
    ...
    const result = getLatestGoalTotals(accounts, balances)
    expect(result.fiTotal).toBe(87000)
  ...
  it('skips inactive accounts', () => {
    ...
    expect(getLatestGoalTotals(accounts, balances).fiTotal).toBe(50000)
  })
