...
const CashflowBarChart: FC<CashflowBarChartProps> = ({ year, yearTransactions, timePeriod, removedCategories, categorySums }) => {
    ...
    <div className="cashflow-bar-wrap">
      ...
      <ResponsiveContainer width="100%" height={340}>
        <BarChart data={data} margin={{ top: 10, right: 20, bottom: 5, left: 10 }} barGap={0} barCategoryGap="20%">
          ...
          />
          <Tooltip
            formatter={(value: number, name: string) => [fmt(Math.abs(value)), name === 'income' ? 'Income' : 'Expense']}
          ...
          <Bar dataKey="income" name="Income" radius={[4, 4, 0, 0]} maxBarSize={48}>
            {data.map((_, i) => <Cell key={i} fill="var(--cashflow-income, #22c55e)" />)}
          </Bar>
          <Bar dataKey="expense" name="Expense" radius={[0, 0, 4, 4]} maxBarSize={48}>
            {data.map((_, i) => <Cell key={i} fill="var(--cashflow-expense, #ef4444)" />)}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
      {/* Net cashflow legend */}
