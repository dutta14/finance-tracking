...
export const GoalsProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const handleDeleteWithUndo = useCallback(
    ...
    [pendingDelete, goals, deleteGoal],
  )
  ...
  const handleDeleteGoal = useCallback((goalId: number): void => handleDeleteWithUndo([goalId]), [handleDeleteWithUndo])

  const handleCopyGwGoals = useCallback(
    (sourcePlanId: number, newPlanId: number): void => {
      gwGoals
    ...
    [gwGoals, createGwGoal],
  )

  const dismissPendingDelete = useCallback((): void => {
    if (!pendingDelete) return
    clearTimeout(pendingDelete.timerId)
    pendingDelete.ids.forEach(id => deleteGoal(id))
    setPendingDelete(null)
  }, [pendingDelete, deleteGoal])
